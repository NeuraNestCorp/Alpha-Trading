...
for i,(eh,el) in enumerate(exp):
    b=(here/f'archive_{i}.b64').read_bytes().strip(); h=hashlib.sha256(b).hexdigest(); print(i,len(b),h,'OK' if (len(b)==el and h==eh) else 'BAD'); parts.append(b)
s=b''.join(parts); print('total',len(s),'mod4',len(s)%4); print('decoded',len(base64.b64decode(s,validate=True)))
